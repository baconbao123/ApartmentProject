...
public class NotiDao {
	...
	// add Noti
	public void insertNoti(int id, int id_toUser, String mess, Boolean status) {
		try (
	...
	public List<Noti> selAllNotiSend(int id) {
		List<Noti> list = new ArrayList<>();
		try (var con = ConnectDB.getConnect(); var cs = con.prepareCall("{call selAllNotiSend(?)}");) {
			cs.setInt(1, id);
			var rs = cs.executeQuery();
			while (rs.next()) {
				Noti noti = new Noti();
				...
				noti.setSender_name(rs.getString("sender_name"));
				noti.setSender_email(rs.getString("sender_email"));
				noti.setReceiver_name(rs.getString("receiver_name"));
				noti.setReceiver_email(rs.getString("receiver_email"));
				noti.setSender_role(rs.getBoolean("sender_role"));
				noti.setMess(rs.getString("message"));
				noti.setStatus(rs.getBoolean("status"));
				// Convert java.util.Date to java.time.LocalDateTime
